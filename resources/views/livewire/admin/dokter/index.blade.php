<div>
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800 uppercase">Kelola Jadwal Dokter</h2>

        <div class="flex space-x-2">
            <a href="{{ route('admin.dokter.index') }}"
                class="px-4 py-2 bg-blue-200 text-blue-800 font-bold rounded-full hover:bg-blue-300 transition">LIHAT</a>
    ...
    <div class="bg-[#1e293b] rounded-lg p-6 shadow-lg">
        <div class="mb-4">
            <input wire:model.live="search" type="text" placeholder="Search"
                class="w-full md:w-1/3 px-4 py-2 rounded-full bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
                ...
                <tbody>
                    @foreach ($dokters as $index => $item)
                        <tr class="border-b border-gray-700 hover:bg-gray-700 transition">
                            <td class="py-3 px-4 text-center">{{ $dokters->firstItem() + $index }}</td>
                            <td class="py-3 px-4">{{ $item->name }}</td>
                            <td class="py-3 px-4">{{ $item->specialization }}</td>
                            <td class="py-3 px-4">{{ $item->patients }}</td>
