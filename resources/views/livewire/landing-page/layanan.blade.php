<div class="pt-24 pb-32 relative overflow-hidden">
    ...
    <div class="mt-12 grid grid-cols-1 md:grid-cols-2 gap-8 px-6 md:px-20 lg:px-40 relative z-10">
        ...
        @foreach ($services as $index => $service)
            <a href="{{ route('landingpage.lihat-layanan', ['id' => $service->id]) }}"
                class="bg-[#A7E2D8] rounded-xl shadow-md p-10 flex items-center gap-6 hover:scale-[1.02] transition cursor-pointer"
                data-aos="fade-up" data-aos-delay="{{ $index * 100 }}">
                <div class="bg-white rounded-full p-4 shadow-md">
                    <x-dynamic-component :component="$service->icon" class="w-10 h-10 text-[#2364AA]" />
                </div>
