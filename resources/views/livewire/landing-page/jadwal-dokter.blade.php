<div class="min-h-screen bg-white">
    ...
    <div class="py-12 text-center">
        <h1 class="text-4xl font-bold text-gray-800">{{ $selectedDoctor->specialization ?? 'Dokter' }}</h1>
    </div>
    ...
    @if ($selectedDoctor)
        ...
        <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div class="relative flex justify-center">
                <div
                    class="bg-gradient-to-b from-[#D7E7EF] to-white rounded-full w-[380px] h-[380px] flex items-center justify-center shadow-lg">
                    <img src="{{ asset($selectedDoctor->image) }}" class="h-[330px] object-contain">
                </div>

                <div class="absolute -bottom-8 bg-white px-6 py-3 shadow-lg rounded-xl border border-gray-200">
                    <p class="font-semibold text-lg">{{ $selectedDoctor->name }}</p>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="flex justify-end mb-6">
                    <select wire:model.live="selectedDoctorId"
                        ...
                        @foreach ($doctors as $d)
                            <option value="{{ $d->id }}">{{ $d->specialization }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex gap-6 flex-wrap justify-end">
                    ...
                    @foreach ($doctors as $d)
                        <div wire:click="selectDoctor({{ $d->id }})"
                            class="cursor-pointer bg-[#E9F3FA] w-48 rounded-xl shadow p-3 border hover:scale-105 transition
                                {{ $selectedDoctorId == $d->id ? 'ring-2 ring-teal-500' : '' }}">
                            <img src="{{ asset($d->image) }}" class="w-24 h-28 object-contain mx-auto">
        ...
        <div class="max-w-7xl mx-auto p-10 grid lg:grid-cols-2 gap-10 mt-24">
            ...
            <div class="bg-white shadow-md rounded-2xl p-8">
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                    <div class="bg-[#D7E7EF] p-4 rounded-xl text-center">
                        ...
                        <p class="text-lg font-bold">{{ $selectedDoctor->patients }}</p>
                    </div>

                    <div class="bg-[#D7E7EF] p-4 rounded-xl text-center">
                        <p class="text-sm text-gray-600">Pengalaman</p>
                        <p class="text-lg font-bold">{{ $selectedDoctor->experience }}</p>
                    </div>

                    <div class="bg-[#D7E7EF] p-4 rounded-xl text-center">
                        <p class="text-sm text-gray-600">Pendidikan</p>
                        <p class="text-lg font-bold">{{ $selectedDoctor->education }}</p>
                    </div>

                    <div class="bg-[#D7E7EF] p-4 rounded-xl text-center">
                        <p class="text-sm text-gray-600">Bahasa</p>
                        <p class="text-lg font-bold">{{ $selectedDoctor->languages }}</p>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap gap-3">
                    @if ($selectedDoctor->skills)
                        @foreach ($selectedDoctor->skills as $skill)
            ...
            <div class="bg-white shadow-md rounded-2xl p-8">
                ...
                @if ($selectedDoctor->schedule)
                    @foreach ($selectedDoctor->schedule as $s)
                        <div class="flex justify-between items-center py-2 last:border-none">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                @if ($s['available'])
                                    <span
                                        class="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs">Tersedia</span>
                                @else
                                    <span class="bg-red-100 text-red-700 px-3 py-1 rounded-full text-xs">Tidak
                                        Ada</span>
                                @endif
                            </div>
                        </div>
    ...
    @else
        <div class="text-center py-20">
            <p class="text-xl text-gray-500">Belum ada data dokter.</p>
        </div>
    @endif
