...
<body class="font-sans antialiased bg-gray-100 flex">
    ...
    <div class="w-64 bg-[#1e293b] text-white min-h-screen flex flex-col fixed left-0 top-0 bottom-0">
        ...
        <nav class="flex-1 px-4 space-y-2">
            <a href="#" class="block px-4 py-2 rounded-lg hover:bg-gray-700 transition {{ request()->routeIs('admin.profile') ? 'bg-gray-700' : '' }}">
                Profile
            </a>
            <a href="{{ route('admin.pengumuman.index') }}" class="block px-4 py-2 rounded-lg hover:bg-gray-700 transition {{ request()->routeIs('admin.pengumuman*') ? 'bg-gray-700 underline decoration-white decoration-2 underline-offset-4' : '' }}">
                Pengumuman
            </a>
